...
import { UserManagerAppProfilesInspireNetMobileProductSelectorSerializer } from './UserManagerAppProfilesInspireNetMobileProductSelectorSerializer';
import { UserRoleSerializer } from './UserRoleSerializer';

// Import derived type serializers (for polymorphic deserialization)

/**
 * Serializer for UserManager - handles JSON serialization and deserialization with type validation
 */
export class UserManagerSerializer {
  /**
   * Deserializes a JSON string into a UserManager instance
   */
  static deserialize(json: string): UserManager {
    const obj = JSON.parse(json);
    return this.fromObject(obj);
  }

  /**
   * Deserializes a plain object into a UserManager instance
   */
  static fromObject(obj: any): UserManager {
    if (!obj) {
      throw new Error('Cannot deserialize null or undefined to UserManager');
    }


    const instance: UserManager = {
      ...
      appProfilesInspireNetMobileProductSelector: obj['AppProfiles|InspireNetMobile_ProductSelector'] as UserManagerAppProfilesInspireNetMobileProductSelector | undefined,
    };
  ...
  /**
   * Deserializes a JSON string into an array of UserManager instances
   */
  static deserializeArray(json: string): UserManager[] {
  ...
  /**
   * Serializes a UserManager instance to a JSON string
   */
  static serialize(instance: UserManager): string {
    return JSON.stringify(this.toObject(instance));
  }

  /**
   * Converts a UserManager instance to a plain object
   */
  static toObject(instance: UserManager): any {
    return {
      'name': instance.name,
  ...
  /**
   * Serializes an array of UserManager instances to a JSON string
   */
  static serializeArray(instances: UserManager[]): string {
